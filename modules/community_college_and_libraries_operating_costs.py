...
hcc_hocobydesign_per_capita = education_expenditure_factors().loc['HCC General Fund','Factor'].squeeze()

def hcc_expenditure():
    total_hcc = pd.DataFrame(columns=years)
    total_hcc = people_generated()[1].mul(hcc_hocobydesign_per_capita)
    total_hcc.loc['Total'] = total_hcc.loc['Columbia':'South East',years].sum()

    return total_hcc

total_hcc_opeb = pd.DataFrame(columns=years)
hcc_opeb_trust_fund_ratio = education_expenditure_factors().loc['HCC OPEB Trust Fund','Factor'].squeeze()

def hcc_opeb_trust_fund():

    people_generated_by_region = people_generated()[1]
    total_hcc_opeb = people_generated_by_region*hcc_opeb_trust_fund_ratio
    total_hcc_opeb = total_hcc_opeb
...
hcl_hocobydesign_per_capita = education_expenditure_factors().loc['Library General Fund','Factor'].squeeze()

def hcl_expenditure():
    total_hcl = pd.DataFrame(columns=years)
    total_hcl = people_generated()[1].mul(hcl_hocobydesign_per_capita)
...
hcl_opeb_trust_fund_ratio = education_expenditure_factors().loc['Library OPEB Trust Fund','Factor'].squeeze()

def hcl_opeb_trust_fund():

    people_generated_by_region = people_generated()[1]
    total_hcl_opeb = people_generated_by_region*hcl_opeb_trust_fund_ratio
    total_hcl_opeb = total_hcl_opeb
