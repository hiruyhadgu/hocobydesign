...
def compiled_revenue_projections():
    ...
    other_local_taxes.loc['Hotel/Motel Tax'] = hotel_motel().loc['Columbia':'South East',years].sum()
    other_local_taxes.loc['Admissions and Amusement Tax'] = admission_amusement()[1].loc['Columbia':'South East',years].sum()


    operating_revenue_projections.loc['Property Taxes'] = prop_tax_by_region()[4].loc['Real Property Tax':'Penalties/Interest Property Taxes',years].sum()
    operating_revenue_projections.loc['Fire Tax'] = project_fire_tax()[0].loc['Columbia':'South East',years].sum()
    operating_revenue_projections.loc['Income Taxes'] = project_income_tax()[0].loc['Columbia':'South East',years].sum()
    operating_revenue_projections.loc['Other Local Taxes']=other_local_taxes.loc['Hotel/Mote Tax':'Admissions and Amusement Tax',years].sum()
    operating_revenue_projections.loc['Other Revenues']=projected_other_revenue.loc['Highway User Tax':'Parking Violations',years].sum()

    capital_revenue_projections.loc['Transfer Taxes']=transfer_tax()[1].loc['Columbia':'South East',years].sum()
    capital_revenue_projections.loc['Road Excise Taxes'] = road_excise_tax().loc['Columbia':'Non Residential',years].sum()
    capital_revenue_projections.loc['School Surcharge Fees']=school_surcharge()[1].loc['Columbia':'South East',years].sum()
...
def cumulative_projections():
    ...
    cumulative_operting_revenue_projections.loc['Recordation Fees'] = recordation()[1].loc['Columbia':'South East',years].sum()
    cumulative_operting_revenue_projections.loc['Total Operating Revenues'] = cumulative_operting_revenue_projections.loc['Property Taxes':'Recordation Fees',years].sum()
    ...
    cumulative_projections = pd.concat([cumulative_operting_revenue_projections,cumulative_capital_revenue_projections])
